class TransactionsController < ApplicationController
	...
	def index
		@transactions = Transaction.where('from_account in (select id from accounts where user_id = ?) or to_account in (select id from accounts where user_id = ?)', session[:user_id], session[:user_id])
	end
